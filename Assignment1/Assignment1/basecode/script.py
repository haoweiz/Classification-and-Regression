...
def ldaLearn(X,y):
    ...
    class_number = int(np.max(y))
    d = np.shape(X)[1]
    means = np.zeros((d,class_number));

    for i in range(1,class_number+1):
        index = np.where(y == i)[0]
        classi = X[index,:]
        means[:,i-1] = np.mean(classi,axis = 0).T
    covmat = np.cov(X.T)
    return means,covmat
...
def qdaLearn(X,y):
    ...
    for i in range(1,class_number+1):
        ...
        classi = X[index,:]
        means[:,i-1] = np.mean(classi,axis = 0).T
        covmats.append(np.cov(classi.T))
...
def ldaTest(means,covmat,Xtest,ytest):
    ...
    class_number = np.shape(means)[1]
    N = np.shape(Xtest)[0]
    ypred = np.zeros((1,N))
    Cov = np.linalg.inv(covmat);
    correct = 0
    for i in range(0,N):
        probability = 0
        classify = 0
        for category in range(1,class_number+1):
            MVN = multivariate_normal.pdf(Xtest[i,:].T, means[:,category-1], Cov)
            if (MVN > probability):
                probability = MVN
                classify = category
        if (classify == ytest[i]):
...
def qdaTest(means,covmats,Xtest,ytest):
    ...
    for i in range(0,N):
        ...
        classify = 0
        for category in range(1,class_number+1):
            MVN = multivariate_normal.pdf(Xtest[i,:].T,means[:,category-1],covmats[category-1])
            if (MVN > probability):
                probability = MVN
                classify = category
        if (classify == ytest[i]):
...
def learnRidgeRegression(X,y,lambd):
    ...
    w = np.dot(np.dot(np.linalg.inv(np.dot(np.transpose(X),X)+lambd*np.identity(np.shape(X)[1])),np.transpose(X)),y)                   
    return w
...
plt.scatter(Xtest[:,0],Xtest[:,1],c=ytest.reshape(np.shape(ytest)[0]))
plt.title('LDA')
